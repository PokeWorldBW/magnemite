	...
	commands: [
		{
			...
			execute(message, args, client) {
				message.channel.send(`Current version is: \`${client.bot.settings.version}\``).catch(error => { console.error(`Error in 'version' command: ${error}`); });
			},
		},
		{
			name: 'uptime',
		...
		},
		{
			...
			execute(message, args, client, props) {
				if (args.length == 0) {
					message.channel.send(Utilities.format(this.help, props)).catch(error => { console.error(`Error in 'info' command: ${error}`); });
					return;
				}
				const data = args[0].toLowerCase();
				switch (data) {
				case 'server':
					message.channel.send(`\`\`\`\nName: ${message.guild.name}\nId: ${message.guild.id}\nOwner: ${message.guild.owner.user.tag}\nTotal Members: ${message.guild.memberCount}\nCreation Time: ${message.guild.createdAt}\nRegion: ${message.guild.region}\n\`\`\``).catch(error => { console.error(`Error in 'info' command: ${error}`); });
					break;
				case 'user':
					message.channel.send(`\`\`\`\nTag: ${message.author.tag}\nId: ${message.author.id}\nAccount Creation Time: ${message.author.createdAt}\n\`\`\``).catch(error => { console.error(`Error in 'info' command: ${error}`); });
					break;
				default:
					message.reply(`I can only get info on the server or users! Please use \`${client.bot.settings.prefix}info server\` or \`${client.bot.settings.prefix}info user\``).catch(error => { console.error(`Error in 'info' command: ${error}`); });
				}
			},
		},
	],
